...
/*        This is a rare case: F is very small. */
	...
	if (f->r == 0. && f->i == 0.) {
	    *cs = 0.;
	    ...
	    r__->r = d__1, r__->i = 0.;
/*           Do fcomplex/real division explicitly with two real divisions */
	    d__1 = gs.r;
	    d__2 = d_imag(&gs);
...
/*        Therefore, CS = F2S/G2S / sqrt( 1 + (F2S/G2S)**2 ) = F2S/G2S */
	*cs = f2s / g2s;
/*        Make sure abs(FF) = 1 */
/*        Do fcomplex/real division explicitly with 2 real divisions */
/* Computing MAX */
...
/*        F2S cannot overflow, and it is accurate */
	...
	f2s = sqrt(g2 / f2 + 1.);
/*        Do the F2S(real)*FS(fcomplex) multiply with two real multiplies */
	d__1 = f2s * fs.r;
	d__2 = f2s * d_imag(&fs);
	z__1.r = d__1, z__1.i = d__2;
	r__->r = z__1.r, r__->i = z__1.i;
